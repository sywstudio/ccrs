if (typeof HdGame == "undefined") {
    ...
    a.poupAjaxComplete = function(c) {
        ...
        a.initChangePoup = function(q) {
            ...
            var r = null;
            $(".poupTitleBox .poupTitleMune").not(".hide").each(function(u, w) {
                var t = $(this).attr("_flag");
                for (var v = 0; v < $("#poupInfoBox .poupMain").length; v++) {
                    var s = $("#poupInfoBox .poupMain").eq(v);
                    if (s.attr("_flag") == t) {
                        s.show();
                        if (r) {
                            r = r.add(s)
        ...
        a.showAwardDetail = function(H) {
            ...
            var y = "";
            r.html("");
            var t = $("#resulte-gift-showMyAward").data("openCode");
            var q = null;
            for (var E = 0; E < H.length; E++) {
                ...
                var F = (s.cbt).replace("-", ".").replace("-", ".");
                var G = (s.cet).replace("-", ".").replace("-", ".");
                A.collectionBtn = J;
                A.addressName = K;
                A.awardCusText = D;
                A.titleNotice = B;
                ...
                if (A.anwei) {
                    w = A.awardLevel;
                    u = A.award;
                    v = (A.cusCode && A.cusCode.length > 0) ? A.cusCode: A.code
                }
                A.awardCode = v;
        ...
        a.openAwardDetail = function(t) {
            ...
            if (g_config.afterLinkModify && a.shouldRegInfo(4, arguments, this)) {
                ...
                if (typeof q.continfo !== "undefined") {
                    if ((g_config.award && ((gameType == 0 || gameType == 4 || gameType == 5) && q.oplink))) {
                        var v = q.continfo;
                        var E = v & 1;
                        var s = v & 2;
                        var B = v & 4;
                        var w = E && !g_config.awardUsername;
                        var u = s && !g_config.awardPhone;
                        var y = B && !g_config.awardAddress;
                        if (w || u || y) {
                    ...
                    if (!g_config.linkInfoType && !g_config.awardUsername && !g_config.awardPhone && !g_config.awardAddress && g_config.award && ((gameType == 0 || gameType == 4) && q.oplink)) {
                        a.aUserInfo.show(arguments, this);
                        $("#addressInput").hide();
                        return
                    }
            ...
            if (t.codeStatus == 4) {
                ...
                if (t.codeStatus == 1) {
                    ...
                    if (t.codeStatus == 3) {
                        ...
                        var C = t.codeBgTime.replace(/\./g, "/");
                        var D = new Date(C).getTime();
                        if (D > A) {
                            var z = Math.ceil((D - A) / (1000 * 60 * 60 * 24));
                            var x = "";
            ...
            G.find(".awardName").text(q.name);
            G.find(".awardSubTitle").text(q.stl);
            G.find(".awardCodeTime em").text(t.awardTypeName);
            G.find(".awardCodeTime .codeBgTime").text(t.codeBgTime);
            $("#codeName").text(t.codeName);
            $("#awardCollectionBtn").text(t.collectionBtn);
            var F = t.gameid || t.gameId;
            G.find(".awardCodeTime .codeEndTime").text(t.codeEndTime);
           // G.find(".codeImg").attr("src", "http://" + a.gameDomain + "/manage/qrCode.jsp?cmd=qrurl&siteUrl=" + a.encodeUrl(t.awardLevel == 900 ? ("id=" + t.id + "&code=" + t.code + "&gameId=" + F) : ("code=" + t.awardCode + "&gameId=" + F)));
            G.find(".code").text(t.awardCode);
            G.find(".code").attr("code", t.code);
            G.find(".codeOptInfo").text(q.opti);
            if (p.afterWxCard) {
                $(".awardCusText").show();
                $(".awardCusBtn").show();
                if (q.awardtype == 5 || q.awardtype == 1 || (q.awardtype == 2 && q.cashtype != 2)) {
                    r.find(".addressLine").hide();
                    G.find(".awardCusText").show();
                    if (q.awardtype == 1) {
                        G.find(".awardCusBtn .text").text("商城地址")
                    } else {
                        G.find(".awardCusBtn .text").text("兑奖地址")
                    }
                    if (q.awardtype == 2 && q.cashtype == 1 && q.onlinect == 1) {
                        $("#QRImg_1").attr("src", q.onlinewxnum);
                        $("#awardCusBtnInfo").attr("href", "###");
                        G.find("#awardCusBtnInfo").off("click").on("click",
                        function() {
                        ...
                        if (q.awardtype == 5 || (q.awardtype == 2 && q.cashtype == 3)) {
                            ...
                            G.find("#attentionCashBtn").off("click").on("click",
                            function() {
                                $("#newAttentionMask").show()
                            })
                        } else {
                    ...
                    if (q.oct) {
                        G.find(".awardCusBtn .text").text(q.btn);
                        G.find("#awardCusBtnInfo").attr("href", q.btl)
                    } else {
                        $(".awardCusBtn").hide()
                    }
                }
                G.find(".awardCusText .text").html(t.awardCusText);
                ...
                r.find(".itemList .notice").html('<pre style="word-wrap: break-word;"></pre>');
                r.find(".itemList .notice pre").text(q.cashinfo)
            } else {
                if (q.optx) {
                    G.find("#awardCusTextInfo").text(q.txc)
                } else {
                    $(".awardCusText").hide()
                }
                if (q.oct) {
            ...
            if (typeof q.cashtype === "undefined" || (q.awardtype != 1 && q.cashtype == 2)) {
                ...
                if (q.attention == 3) {
                    ...
                    $("#awardCusBtnInfo").off("touchend").on("touchend",
                    function() {
                        a.logDog(1000086);
                        $("#newAttentionMask").show()
                    })
        ...
        function h(q) {
            ...
            if (_manage) {
                a.homePoup(q);
                var u = [];
                var t = ["lvox、hubert", '<span style="color:#ffffff">magazine</span>、tina', "monica、hth"];
                var s = $(".regAwardList");
        ...
        $("#awardContactInfo .updateBtn").on("touchend",
        function() {
            a.aUserInfo.show()
        })
    })(); (function() {
    ...
    a.showLotsWait = function(c, e) {
        ...
        g.init = function(h) {
            ...
            $(".resule-status-home , .resule-gift-home, .resule-gift-home2").on("touchend",
            function() {
                event.preventDefault();
    ...
    a.otherAjaxComplete = function() {
        ...
        function d(j) {
            return function() {
                ...
                } else { (typeof l === "string" || l.nodeType) && (l = $(l));
        ...
        $.fn.extend({
            ...
            awardSlide: function(j) {
                if (typeof j === "object") {
                    ...
                    var j = $.extend(k, j);
                    $(this).data("awardSlideOptions", j);
                    this.each(function() {
                        $(this).show();
                        var l = $(this),
                        m = l.children("ul");
                        var n = m.find("li");
                        m.append("<li class='clone'></li>");
                        m.find("li").eq(0).children().clone().appendTo(".clone");
                        m.find("li").eq(0).css("margin-left", "320px");
                        var o = 0;
                        m.css("width", "10000%");
                        n.find("div").css({
                            "margin-left": j.margin_left,
                            "margin-right": j.margin_right
                        });
                        $(this).data("awardSlide-marquee", setInterval(p, j.speed));
                        function p() {
                            l.scrollLeft(++o);
                            if (o == n.outerWidth(true)) {
                                o = 320
                            }
                        }
                    })
                    ...
                    if (j === "stop") {
                        ...
                        this.each(function() {
                            ...
                            $(this).data("awardSlide-marquee", null)
                        })
                    } else {
                        if (j === "sleep") {
                            this.each(function() {
                            ...
                            if (j === "wakeup") {
                                $(this).show();
                                var j = $(this).data("awardSlideOptions");
    ...
    a.tlog = (function() {
        ...
        $(function() {
            ...
            d.find("#log_close").on("touchstart",
            function() {
                g.hide();
                d.hide();
                e.empty();
            ...
            d.find("#log_min").on("touchstart",
            function() {
                g.show();
                d.hide()
            });
            d.find("#log_share").on("touchstart",
            function() {
    ...
    a.initSound = function(e, f, d) {
        ...
        var g = {
            ...
            readyPlay: function(m, n, j) {
                ...
                var i = c[m];
                if (!i) {
                    a.tlog("Sound_pause_err", "这个key:" + m + " 不存在!");
                    return this
                }
                var k = this;
                k.play(m, n, j);
                k.onReady(m,
                function() { ! i.playing && k.play(m, n, j)
                })
            },
            pause: function(j) {
                if (_manage) {
    ...
    a.initJsFoot = function(d) {
        $(function() {
            ...
            function(m) {
                ...
                a.openAwardDetail.call(this, m)
            });
            $(".awardCloseIcon").on("touchstart",
            function() {
            ...
            $(".ticketitle .closeBtn").on("touchstart",
            function() {
            ...
            if (!_manage) {
                ...
                $("#startBtnImg,.startBtnImg").on("touchend",
                function(m) {
				//alert(1);
				//2017年6月16日 05:59:02 jack start
				var jack_status = startgamejack();
				if(jack_status == false){
					return false;
				}
				//2017年6月16日 05:59:08 jack end
                ...
                a.initModuleLayer([["#joinNumLine", 0, "joinNum", null, true], ["#gameTopBar", -2, "gameTopBar", null, "样式设置"], ["#logoImg", 2, "logoImg"], ["#awardLine,#selfAwardLine", 1, "awardSetting"], ["#gameDateTime", 0, "gameTime"], ["#explainBox,#awardExplainBox,#resule-gift-explain", 0, "explain"], ["#resule-status-count", 2, "drawInfo"], [".hostLine", 2, "hostLine"], ["#skillLine", 1, "skillLine"], ["#rankBox", 1, "rankBox"], [".resule-status-minscore", 2, "minscore"], [".menuBtnBox", 2, "menuBtn"], ["#awardDetailBox .awardName", 1, "awardName"], ["#awardDetailBox .awardCodeTime", 1, "awardTime"], ["#awardDetailBox .awardSubTitle", 1, "awardSubTitle"], ["#awardDetailBox .awardCusBtn", 1, "awardCusBtn"], ['.home .soundIcon:not(".soundIconPlay")', -2, "#limitRange"], ["#awardDetailBox .codeOptInfo", 1, "codeOptInfo"], ["#awardDetailBox .webCashBtn", 1, "webCashBtn"], ["#resule-gift-sucImg", -1, "", [{
                ...
                if (!g_config.afterWxCard) {
                    a.initModuleLayer([["#awardDetailBox .awardCusText", 1, "awardCusText"], [".codeImg", 1, "codeImg"], ])
                }
                $(".titleImg").addClass("abs");
                a.moduleSlaveImgResize([[$("#titleImg"), {
                ...
                "#limitRange"], ]);
                a.moduleDraggale([["#titleImg", true], ["#startBtnImg", true], ["#logoImg", true, "#limitRange"], ["#ruleImg", false, "#limitRange"], ["#joinNumLine", false], ["#drawInfo", false], ['.home .soundIcon:not(".soundIconPlay")', false], ]);
                i($("#logoImg").closest(".ui-wrapper"), 80);
                i($("#titleImg").closest(".ui-wrapper"), 70);
                i($("#drawImg").closest(".ui-wrapper"), 80);
                i($("#ruleImg"), 75);
                function i(n, m) {
                    n.css("z-index", m)
                }
                ...
                $("#resule-gift-sucImg").setResizableOption({
                    ...
                    dxRatio: 2,
                    containment: false,
                }).addResizableFn("start",
                function(m, n) {
                    $(this).add($(this).parent()).css({
                        "max-height": (parseInt($("#resule-gift-scrollWrap").css("height")) - $(this).offset().top) / g_rem + "rem"
                    })
                }).addResizableFn("resize",
                function(n, o) {
                    $(this).add($(this).parent()).css({
                        left: 0,
                        top: 0,
                    });
                    var m = parseInt($("#resule-gift-scrollWrap").css("height")) - $(this).offset().top;
                    if ($(this).height() > m) {
                        $(this).add($(this).parent()).css({
                            "max-height": m / g_rem + "rem"
                        })
                    }
                });
                ...
                function(m) {
                    var n = "";
                    $("#ruleBox .poupLine").each(function(p, q) {
                        var o = $(this).data("sortkey");
                        if (o) {
                            n += o
                        }
                    });
                    ...
                    if (hg.sound.list && hg.sound.list.length > 0) {
                        var m = ".home,.gameBgBox";
                        if (g_config.style != 0 && g_config.style != 1 && g_config.style != 2 && g_config.style != 24) {
                            m = ".gameBgBox"
                        }
                        if (g_config.style == 34) {
                            m = "#gameTopBar,#gameLayerBox1,#gameLayerBox2,#gameLayerBox3"
                        }
                        if (g_config.style == 45) {
                            m = ".gameInfoBoxMain"
                        }
                        if (g_config.style == 48) {
                            m = "#gameContent,.friendEnterBoxer"
                        }
                        if (g_config.style == 9) {
                            m = "#openBox"
                        }
                        parent.Edit.addEditBtn("编辑音效", m,
                        function() {
                            parent.Edit.showEditByGame( - 4, "", []);
                            return false
                        })
                    }
                    g_config.style != 49 && parent.Edit.addEditBtn("编辑背景", "#ruleImg,.home,.gameBgBox",
                    function() {
                        var o = [];
                        var r = {};
                        if ($(".home").is(":visible")) {
                            r.from = "#homeBg";
                            r.defSrc = d.homeBgPath_def
                        } else {
                            if ($(".gameBgBox2").is(":visible")) {
                                r.from = "#gameBg2";
                                r.defSrc = d.gameBg2Path_def
                            } else {
                                if ($(".gameBgBox").is(":visible")) {
                                    r.from = "#gameBg";
                                    r.defSrc = d.gameBgPath_def
                                }
                            }
                        }
                        if (hg.edit && hg.edit.bgList) {
                            var q, p;
                            for (var n = hg.edit.bgList.length - 1; n >= 0; n--) {
                                q = hg.edit.bgList[n];
                                p = $(q.from);
                                if (p.length > 0 && p.eq(0).is(":visible")) {
                                    r = q;
                                    break
                                }
                            }
                        ...
                        r.limit = "500k";
                        o.push(r);
                        parent.Edit.showEditByGame( - 1, "", o);
                        return false
                    })
                })();
